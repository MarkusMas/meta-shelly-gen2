...
#### Lables
- SWITCH 0: contains the current status of the channel including power status (ON/OFF), power consumption (Wattage), current (Amps) and voltage (Volt) . For example: "SWITCH 0: ON 50W 2A 228.6V". Also after each button press the actual switching state of the relay will be displayed for a split second. For example: "SWITCH 0: TURNED ON", "SWITCH 0: STAYS ON", etc .
